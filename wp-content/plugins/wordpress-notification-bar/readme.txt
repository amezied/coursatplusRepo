=== Plugin Name ===
Contributors: seedprod, johnnytee
Donate link: http://seedprod.com/
Tags: message, floating bar, notice, notification, sticky header, special offer, discount offer, offer, important, notification bar, attention bar, highlight bar
Requires at least: 3.0.0
Tested up to: 5.3
Stable tag: 1.3.10

A quick and easy notification bar and call to action for your site.

== Description ==

A quick and easy **notification bar** and **call to action** for your site.

*   [Upgrade to the Notification Bar Pro Version Now! &raquo;](https://www.seedprod.com/wordpress-notification-bar/?utm_source=wordpress-notification-bar-plugin&utm_medium=banner&utm_campaign=wordpress-notification-bar-link-from-wordpress.org "Notifification Bar Pro Version")


### Benefits, Features and Options:

*   Quick & easy setup
*   Custom message and call to action button
*	Sticky Notification Bar
*	Choose any color to match your site
*   i18n Support
*   Multisite Support

### Pro Version Features
* All the Free Features Plus...
* Create Unlimted Bars per Site
* Capture Email Leads
* Custom Countdowns
* Social Media Icons
* More Color Options
* Google Fonts Support
* Support
* Plus Much More...


*   [Upgrade to the Notification Bar Pro Version Now! &raquo;](https://www.seedprod.com/wordpress-notification-bar/?utm_source=wordpress-notification-bar-plugin&utm_medium=banner&utm_campaign=wordpress-notification-bar-link-from-wordpress.org "Notifification Bar Pro Version")

**Included Translations:**

* Czech(cz_CZ) by [Lukas Prelovsky](http://lukasprelovsky.sk)
* Danish(da_DK) by [Alexander Leo-Hansen](http://alexanderleohansen.dk)
* German(de_DE) by Daniel Hoffmann
* Indonesia(id_ID) by [Ridha Harwan](https://tarjiem.com/)
* Persian(fa_IR) by [rushearn](http://rushearn.com)
* Turkish(tr_TR) by [Hakan Er](http://hakanertr.wordpress.com)
* Russian(ru_RU) by Alexander Samsonov
* Slovak(sk_SK) by [Lukas Prelovsky](http://lukasprelovsky.sk)
* Swedish(sv_SE) by [Tobias](http://comclick.se)

== Screenshots ==

1. Front End
2. Back End


== Changelog ==
= 1.3.10 =
* Fixed php warnings

= 1.3.9 =
* Added Italian Translation

= 1.3.8 =
* New Pro Version Released

= 1.3.7 =
* Changed z-index so the bar does not cover the WP Admin bar

= 1.3.6 =
* PHP 7 Compatibility

= 1.3.5 =
* Language Packs

= 1.3.4 =
* Added: Danish Translations

= 1.3.3 =
* Added: Swedish Translations

= 1.3.2 =
* Added: Indonesia Translations

= 1.3.1 =
* Added: Slovak and Czech Translations

= 1.3.0 =
* Added:  Responsive

= 1.2.4 =
* Added:  Russian Translation

= 1.2.3 =
* Added:  Turkish Translation

= 1.2.2 =
* Added: Persian Translation

= 1.2.1 =
* Fixed bug that appeared in 1.2.0

= 1.2.0 =
* Added German Translation

= 1.1.3 =
* Update compatability to 3.5.0

= 1.1.2 =
* Fized languages file path error

= 1.1.1 =
* Fized a bug in IE8 causing display issues

= 1.1.0 =
* Added the ability open the link in a new window
* Updated POT file

= 1.0.0 =
* Initial Release


== Upgrade Notice ==
= 1.3.9 =
* Added Italian Translation

= 1.3.7 =
* Changed z-index so the bar does not cover the WP Admin bar

= 1.3.6 =
* PHP 7 Compatibility

= 1.3.5 =
* Language Packs

= 1.3.3 =
* Added: Swedish Translations

= 1.3.2 =
* Added: Indonesia Translations

= 1.3.1 =
* Added: Slovak and Czech Translations

= 1.3.0 =
* Added:  Responsive

= 1.2.4 =
* Added:  Russian Translation

= 1.2.3 =
* Added:  Turkish Translation

= 1.2.2 =
* Added: Persian Translation

= 1.2.1 =
* Fixed bug that appeared in 1.2.0

= 1.2.0 =
* Added German Translation

= 1.1.3 =
* Update compatability to 3.5.0

= 1.1.2 =
* Fized languages file path error

= 1.1.1 =
* Fized a bug in IE8 causing display issues

= 1.1.0 =
* Added the ability open the link in a new window
* Updated POT file

= 1.0.0 =
* Initial Release
