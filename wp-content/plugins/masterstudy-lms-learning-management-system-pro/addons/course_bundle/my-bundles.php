...
class STM_LMS_My_Bundles
{
    ...
    function stm_lms_delete_bundle() {
        ...
        if (!STM_LMS_My_Bundle::check_author($bundle_id, get_current_user_id())) die;
    ...
    function stm_lms_change_bundle_status()
    {
        ...
        if ($bundle_status === 'draft' && STM_LMS_My_Bundle::get_available_quota()) {
            $post_status = 'publish';
        }
    ...
    function stm_lms_get_user_bundles()
    {
    ...
    static function get_bundles($args = array(), $public = false)
    {
        $per_page = self::per_page();
        if(!empty($args['posts_per_page'])) $per_page = $args['posts_per_page'];

        $page = (!empty($_GET['page'])) ? intval($_GET['page']) : 0;
        if(!empty($args['stm_lms_page'])) $page = $args['stm_lms_page'];
        ...
        if (!is_user_logged_in() && !$public) return $r;
        ...
        if ($q->have_posts()) {
            while ($q->have_posts()) {
                ...
                if (!empty($bundle_courses)) $courses = array_unique(array_merge($courses, $bundle_courses));
                ...
                $r['posts'][] = array(
                    ...
                    'edit_url' => STM_LMS_Course_Bundle::url() . '/' . $id,
                    'raw_price' => $price,
