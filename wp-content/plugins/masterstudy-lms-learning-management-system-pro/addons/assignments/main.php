...
class STM_LMS_Assignments
{
    ...
    function __construct()
    {
        ...
        add_filter('stm_lms_boxes', array($this, 'stm_lms_boxes'), 10, 1);
        add_filter('stm_lms_fields', array($this, 'stm_lms_fields'), 10, 1);
    ...
    function has_current_assignments($item_id)
    {
    ...
    static function has_passed_assignment($item_id)
    {
        $user = STM_LMS_User::get_current_user();
        if (empty($user['id'])) wp_redirect(remove_query_arg(array('start_assignment', 'course_id'), self::get_current_url()));
    ...
    function stm_lms_settings()
    {
        return apply_filters('stm_lms_assignments_settings', array(
            'id' => 'stm_lms_assignments_settings',
            'args' => array(
                'stm_lms_assignments_settings' => array(
                    'credentials' => array(
    ...
    function stm_lms_settings_page_view()
    {
        ...
        foreach ($metabox['args']['stm_lms_assignments_settings'] as $section_name => $section) {
            foreach ($section['fields'] as $field_name => $field) {
                $default_value = (!empty($field['value'])) ? $field['value'] : '';
                $metabox['args']['stm_lms_assignments_settings'][$section_name]['fields'][$field_name]['value'] = (!empty($settings[$field_name])) ? $settings[$field_name] : $default_value;
            }
        }
        $title = esc_html__('Assignment settings', 'masterstudy-lms-learning-management-system-pro'); ?>
        <script>
    ...
    function stm_lms_fields($fields)
    {
        ...
        $fields['stm_student_assignment'] = array(
            'section_group' => array(
                'name' => esc_html__('Assignment', 'masterstudy-lms-learning-management-system-pro'),
                'fields' => array(
                    'try_num' => array(
                        'type' => 'number',
    ...
    /*AJAX*/
    static function stm_lms_upload_assignment_file()
    {
        ...
        if($filesize > $max_file_size) {
            wp_send_json(array(
                'error' => true,
                'message' => esc_html__('File is too large. Split it up to several archives.', 'masterstudy-lms-learning-management-system-pro')
            ));
        }
    ...
    function stm_lms_accept_draft_assignment() {
        ...
        if ($user_id === $assignment_student_id) {
            ...
            STM_LMS_Helpers::send_email($user['email'], esc_html__('New assignment', 'masterstudy-lms-learning-management-system-pro'), $message);
