...
class STM_LMS_Point_System_Statistics
{
    ...
    function stats_page_view()
    {
        STM_LMS_Templates::show_lms_template('points/admin/stats');
    }

    static function users_per_page()
    {
        return 50;
    }

    static function users($args = array())
    {
    ...
    function stm_lms_get_point_users() {
        $per_page = self::users_per_page();
        $args = array();

        if(!empty($_GET['page'])) $args['offset'] = ($per_page * intval($_GET['page'])) - $per_page;
        if(!empty($_GET['s'])) $args['search'] = '*'.sanitize_text_field($_GET['s']).'*';
    ...
    function stm_lms_get_user_points_history_admin() {
    ...
    function stm_lms_change_points() {
    ...
    function stm_lms_delete_points() {
