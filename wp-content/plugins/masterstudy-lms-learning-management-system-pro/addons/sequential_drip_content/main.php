...
class STM_LMS_Sequential_Drip_Content
{
    ...
    function __construct()
    {
        ...
        add_filter('stm_lms_prev_status', array($this, "prev_status"), 10, 3);

        add_filter('stm_lms_boxes', array($this, 'sequential_boxes'));
        add_filter('stm_lms_fields', array($this, 'sequential_fields'));
    ...
    function stm_lms_settings()
    {
        return apply_filters('stm_lms_sequential_drip_content_settings', array(
            'id' => 'stm_lms_sequential_drip_content_settings',
            'args' => array(
                'stm_lms_sequential_drip_content_settings' => array(
                    'credentials' => array(
    ...
    function prev_status($status, $course_id, $item_id)
    {
    ...
    function is_parent_passed($course_id, $item_id, $get_parent = false)
    {
                    ...
                    if (!empty($drip_content_single['childs'])) {
                        foreach ($drip_content_single['childs'] as $drip_content_child) {
                            if ($item_id === $drip_content_child['id']) {
                                ...
                                $check_parent_passed = STM_LMS_Lesson::is_lesson_completed('', $course_id, $parent);
                                if ($get_parent) {
