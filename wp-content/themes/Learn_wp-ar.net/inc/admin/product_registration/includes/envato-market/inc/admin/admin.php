...
if ( ! class_exists( 'Envato_Market_Admin' ) && class_exists( 'Envato_Market' ) ) :
	...
	class Envato_Market_Admin {
		...
		public function __clone() {
			_doing_it_wrong( __FUNCTION__, esc_html__( 'Cheatin&#8217; huh?', 'masterstudy' ), '1.0.0' );
		}
		...
		public function maybe_delete_transients() {
			if ( isset( $_POST[ envato_market()->get_option_name() ] ) ) {
				...
				if ( isset( $_POST['_wpnonce'] ) && ! wp_verify_nonce( $_POST['_wpnonce'], envato_market()->get_slug() . '-options' ) ) {
		 			wp_die( __( 'You do not have sufficient permissions to delete transients.', 'masterstudy' ) );
				}
		...
		public function admin_enqueue_script() {
			...
			$exports = array(
				...
				'i18n' => array(
					'save' => __( 'Save', 'masterstudy' ),
					'remove' => __( 'Remove', 'masterstudy' ),
					'cancel' => __( 'Cancel', 'masterstudy' ),
					'error' => __( 'An unknown error occurred. Try again.', 'masterstudy' ),
				),
			);
		...
		public function render_templates() {
			...
			<script type="text/html" id="tmpl-envato-market-auth-check-button">
				<a href="<?php echo esc_url( add_query_arg( array( 'authorization' => 'check' ), envato_market()->get_page_url() ) ); ?>" class="button button-secondary auth-check-button" style="margin:0 5px"><?php esc_html_e( 'Test API Connection', 'masterstudy' ); ?></a>
			</script>

			<script type="text/html" id="tmpl-envato-market-item">
				<li data-id="{{ data.id }}">
					<span class="item-name"><?php esc_html_e( 'ID', 'masterstudy' ); ?>: {{ data.id }} - {{ data.name }}</span>
					<button class="item-delete dashicons dashicons-dismiss">
						<span class="screen-reader-text"><?php esc_html_e( 'Delete', 'masterstudy' ) ?></span>
					</button>
			...
			<script type="text/html" id="tmpl-envato-market-dialog-remove">
				<div id="envato-market-dialog-remove" title="<?php esc_attr_e( 'Remove Item', 'masterstudy' ) ?>">
					<p><?php esc_html_e( 'You are about to remove the connection between the Envato Market API and this item. You cannot undo this action.', 'masterstudy' ) ?></p>
				</div>
			</script>

			<script type="text/html" id="tmpl-envato-market-dialog-form">
				<div id="envato-market-dialog-form" title="<?php esc_attr_e( 'Add Item', 'masterstudy' ) ?>">
					<form>
						<fieldset>
							<label for="token"><?php esc_html_e( 'Token', 'masterstudy' ) ?></label>
							<input type="text" name="token" class="widefat" value="" />
							<p class="description"><?php esc_html_e( 'Enter the Envato API Personal Token.', 'masterstudy' ); ?></p>
							<label for="id"><?php esc_html_e( 'Item ID', 'masterstudy' ) ?></label>
							<input type="text" name="id" class="widefat" value="" />
							<p class="description"><?php esc_html_e( 'Enter the Envato Item ID.', 'masterstudy' ); ?></p>
						...
						<div class="envato-card-top">
							...
							<div class="column-name">
								<h4>
									...
									<span class="version" aria-label="<?php esc_attr_e( 'Version %s', 'masterstudy' ); ?>"><?php esc_html_e( 'Version', 'masterstudy' ); ?> {{ data.version }}</span>
								</h4>
							</div>
							<div class="column-description">
								...
								<p class="author">
									<cite><?php esc_html_e( 'By', 'masterstudy' ); ?> {{ data.author }}</cite>
								</p>
							</div>
						</div>
						<div class="envato-card-bottom">
							<div class="column-actions">
								<a href="{{{ data.install }}}" class="button button-primary">
									<span aria-hidden="true"><?php esc_html_e( 'Install', 'masterstudy' ); ?></span>
									<span class="screen-reader-text"><?php esc_html_e( 'Install', 'masterstudy' ); ?> {{ data.name }}</span>
								</a>
							</div>
						</div>
					</div>
		...
		public function register_settings() {
			...
			add_settings_section(
				...
				__( 'Global OAuth Personal Token', 'masterstudy' ),
			...
			add_settings_section(
				...
				__( 'Single Use OAuth Personal Tokens', 'masterstudy' ),
			...
			add_settings_field(
				'items',
				__( 'Envato Market Items', 'masterstudy' ),
		...
		public function install_plugin( $plugin ) {
			if ( ! current_user_can( 'install_plugins' ) ) {
				$msg = '
				<div class="wrap">
					<h1>' . __( 'Installing Plugin...', 'masterstudy' ) . '</h1>
					<p>' . __( 'You do not have sufficient permissions to install plugins on this site.', 'masterstudy' ) . '</p>
					<a href="' . esc_url( 'admin.php?page=' . envato_market()->get_slug() . '&tab=plugins' ) . '">' . __( 'Return to Plugin Installer', 'masterstudy' ) . '</a>
				</div>';
				wp_die( $msg );
			}
			...
			if ( empty( $array_api ) ) {
				...
				<div class="wrap">
					<h1>' . __( 'Installing Plugin...', 'masterstudy' ) . '</h1>
					<p>' . __( 'An error occurred, please check that the item ID is correct.', 'masterstudy' ) . '</p>
					<a href="' . esc_url( 'admin.php?page=' . envato_market()->get_slug() . '&tab=plugins' ) . '">' . __( 'Return to Plugin Installer', 'masterstudy' ) . '</a>
				</div>';
				wp_die( $msg );
			}

			$title = sprintf( __( 'Installing Plugin: %s', 'masterstudy' ), $api->name . ' ' . $api->version );
		...
		public function install_theme( $theme ) {
			if ( ! current_user_can( 'install_themes' ) ) {
				$msg = '
				<div class="wrap">
					<h1>' . __( 'Installing Theme...', 'masterstudy' ) . '</h1>
					<p>' . __( 'You do not have sufficient permissions to install themes on this site.', 'masterstudy' ) . '</p>
					<a href="' . esc_url( 'admin.php?page=' . envato_market()->get_slug() . '&tab=themes' ) . '">' . __( 'Return to Theme Installer', 'masterstudy' ) . '</a>
				</div>';
				wp_die( $msg );
			}
			...
			if ( empty( $array_api ) ) {
				...
				<div class="wrap">
					<h1>' . __( 'Installing Theme...', 'masterstudy' ) . '</h1>
					<p>' . __( 'An error occurred, please check that the item ID is correct.', 'masterstudy' ) . '</p>
					<a href="' . esc_url( 'admin.php?page=' . envato_market()->get_slug() . '&tab=themes' ) . '">' . __( 'Return to Plugin Installer', 'masterstudy' ) . '</a>
				</div>';
				wp_die( $msg );
			}
			...
			$title = sprintf( __( 'Installing Theme: %s', 'masterstudy' ), $api->name . ' ' . $api->version );
			...
			// Must have the upgrader & skin.
			stm_include_file( envato_market()->get_plugin_path() . '/inc/admin/upgrader.php' );
			stm_include_file( envato_market()->get_plugin_path() . '/inc/admin/upgrader-skins.php');
		...
		public function ajax_upgrade_theme() {
			...
			if ( $theme_data->exists() && $theme_data->get( 'Version' ) ) {
				$status['oldVersion'] = sprintf( __( 'Version %s', 'masterstudy' ), $theme_data->get( 'Version' ) );
			}

			if ( ! current_user_can( 'update_themes' ) ) {
				$status['error'] = __( 'You do not have sufficient permissions to update themes for this site.', 'masterstudy' );
		 		wp_send_json_error( $status );
			}

			stm_include_file( ABSPATH . 'wp-admin/includes/class-wp-upgrader.php' );
			...
			if ( is_array( $result ) && ! empty( $result[ $theme ] ) ) {
				...
				if ( $theme_data->exists() && $theme_data->get( 'Version' ) ) {
					$status['newVersion'] = sprintf( __( 'Version %s', 'masterstudy' ), $theme_data->get( 'Version' ) );
				}
				...
				$status['error'] = __( 'Unable to connect to the filesystem. Please confirm your credentials.', 'masterstudy' );
		...
		public function ajax_add_item() {
			if ( ! check_ajax_referer( self::AJAX_ACTION, 'nonce', false ) ) {
				...
				wp_send_json_error( array( 'message' => __( 'The Token is missing.', 'masterstudy' ) ) );
			} elseif ( empty( $_POST['id'] ) ) {
				wp_send_json_error( array( 'message' => __( 'The Item ID is missing.', 'masterstudy' ) ) );
			}
			...
			if ( false === $request ) {
				wp_send_json_error( array( 'message' => __( 'The Token or Item ID is incorrect.', 'masterstudy' ) ) );
			}

			if ( false === envato_market()->api()->download( $_POST['id'], $args ) ) {
				wp_send_json_error( array( 'message' => __( 'The item cannot be downloaded.', 'masterstudy' ) ) );
			}
			...
			wp_send_json_error( array( 'message' => __( 'An unknown error occurred.', 'masterstudy' ) ) );
		}
		...
		public function render_admin_callback() {
			stm_include_file( envato_market()->get_plugin_path() . 'inc/admin/view/callback/admin.php' );
		}
		...
		public function render_oauth_section_callback() {
			stm_include_file( envato_market()->get_plugin_path() . 'inc/admin/view/callback/section/oauth.php' );
		}
		...
		public function render_items_section_callback() {
			stm_include_file( envato_market()->get_plugin_path() . 'inc/admin/view/callback/section/items.php' );
		}
		...
		public function render_token_setting_callback() {
			stm_include_file( envato_market()->get_plugin_path() . 'inc/admin/view/callback/setting/token.php' );
		}
		...
		public function render_items_setting_callback() {
			stm_include_file( envato_market()->get_plugin_path() . 'inc/admin/view/callback/setting/items.php' );
		}
		...
		public function render_intro_partial() {
			stm_include_file( envato_market()->get_plugin_path() . 'inc/admin/view/partials/intro.php' );
		}
		...
		public function render_tabs_partial() {
			stm_include_file( envato_market()->get_plugin_path() . 'inc/admin/view/partials/tabs.php' );
		}
		...
		public function render_settings_panel_partial() {
			stm_include_file( envato_market()->get_plugin_path() . 'inc/admin/view/partials/settings.php' );
		}
		...
		public function render_themes_panel_partial() {
			stm_include_file( envato_market()->get_plugin_path() . 'inc/admin/view/partials/themes.php' );
		}
		...
		public function render_plugins_panel_partial() {
			stm_include_file( envato_market()->get_plugin_path() . 'inc/admin/view/partials/plugins.php' );
		}
		...
		public function render_success_notice() {
			stm_include_file( envato_market()->get_plugin_path() . 'inc/admin/view/notice/success.php' );
		}
		...
		public function render_success_no_items_notice() {
			stm_include_file( envato_market()->get_plugin_path() . 'inc/admin/view/notice/success-no-items.php' );
		}
		...
		public function render_success_single_use_notice() {
			stm_include_file( envato_market()->get_plugin_path() . 'inc/admin/view/notice/success-single-use.php' );
		}
		...
		public function render_error_notice() {
			stm_include_file( envato_market()->get_plugin_path() . 'inc/admin/view/notice/error.php' );
		}
		...
		public function render_error_single_use_notice() {
			stm_include_file( envato_market()->get_plugin_path() . 'inc/admin/view/notice/error-single-use.php' );
		}
	}
